...
- Added official stateful LLM context DTOs, a synchronized process store, and a JDK HttpServer Action Server coordinator with owner scoping, CAS reservations, lifecycle actions, true asynchronous execution, cancellation, and all 19 shared conformance vectors.
...
- Upgraded MessagePack to 0.9.11, Jackson to 2.18.9, and BouncyCastle to 1.84, clearing the published runtime dependency advisories found by the alpha.18 package gate.

## [1.0.0-alpha.17] — 2026-08-02

### Added

- Port the reference server surface into the Java SDK: NCP native transport, NWP action/complex/memory nodes and bidirectional bridges, NIP CA services and full verification, NOP orchestration, daemon observability, and telemetry.
...
- Declare the JUnit platform launcher explicitly for Gradle 9 compatibility and enable compiler lint checks.
